    ...
    public partial class AddParticipantWindow : Window // Код-бихайнд для окна AddParticipantWindow.
    {
        ...
        private void BtnOk_Click(object sender, RoutedEventArgs e) // Обработчик нажатия кнопки OK.
        {
            ...
            if (_existingParticipant != null) // Проверяет, передан ли участник для редактирования.
            {
                ...
                // Изменить баланс и жадность
                // Для изменения баланса мы можем просто установить поле напрямую
                // (но в классе есть метод AddMoney, поэтому сначала обнулим, затем прибавим)
                // Допустим, мы хотим полностью заменить баланс на введённое значение:
                // сбросить старый баланс и добавить новый
                // Но безопаснее: пусть InitialBalance — это новое значение,
                // значит убираем старый остаток и кладём новый:
